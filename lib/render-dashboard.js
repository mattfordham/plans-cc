...
function dirtyMark(dirty) {
  if (dirty === true) return fg(COLORS.dirty, "●");
  if (dirty === false) return fg(COLORS.clean, "✓");
  return "";
}
...
function buildHeaderContent({ branches, project }) {
  ...
  if (b.root) {
    const mark = dirtyMark(b.rootDirty);
    lines.push(
      " " + fg(COLORS.branch, "  branch: " + b.root) + (mark ? " " + mark : "")
    );
  }
...
function buildFooterContent() {
  const legend =
    ...
    fg(COLORS.kept, "⌂") + " Kept Worktree";
  const repoLegend =
    fg(COLORS.clean, "✓") + " clean   " + fg(COLORS.dirty, "●") + " dirty";
  const hint = fg(COLORS.hint, "press q or Ctrl+C to exit");
  return legend + "    " + repoLegend + "\n" + hint;
}

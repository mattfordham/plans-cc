...
const STAT_CARDS = [
  ...
  { key: "review", label: "review", color: COLORS.review },
  { key: "completed", label: "completed", color: COLORS.completed },
];

const CARD_GAP = 4;

// Compact 3-row block-letter banner spelling "plans"
const BANNER_LINES = [
  " ┌─┐┬  ┌─┐┌┐┌┌─┐",
  " ├─┘│  ├─┤│││└─┐",
  " ┴  ┴─┘┴ ┴┘└┘└─┘",
...
function buildStatsContent(summary) {
  // Each card's column width is its label length; the value is centered under it.
  const gap = " ".repeat(CARD_GAP);
  let labelRow = "  ";
  let valueRow = "  ";
  STAT_CARDS.forEach((card, i) => {
    const width = card.label.length;
    const valueText = centerIn(String(summary[card.key] || 0), width);
    const prefix = i === 0 ? "" : gap;
    labelRow += prefix + fg(card.color, padRight(card.label, width));
    valueRow += prefix + fg(card.color, valueText);
  });
  return labelRow + "\n" + valueRow;
}

function buildHeaderContent(branch) {
  const lines = [];
  for (const line of BANNER_LINES) {
    lines.push(" " + fg(COLORS.banner, line));
  }
  if (branch) {
    lines.push(" " + fg(COLORS.branch, " branch: " + branch));
  }
  return lines.join("\n");
}
...
function updateLayout(widgets, { tasks, summary, branch }) {
  const { headerBox, statsBox, tasksBox, footerBox } = widgets;
  headerBox.setContent(buildHeaderContent(branch));

...
function buildHeaderContent({ branch, project }) {
  const lines = [];
...
function updateLayout(widgets, { tasks, summary, branch, project }) {
  const { headerBox, statsBox, tasksBox, footerBox } = widgets;
  headerBox.setContent(buildHeaderContent({ branch, project }));
